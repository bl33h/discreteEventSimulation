...
def process (name, env, memory, cpu, arrival, instructionsq, ramq): # Elementos que conforman a un proceso.
    ...
    print('%s \nProceso en cola [NEW llegada] -> %d cantidad ram requerida %d, disponible %d' % (name, env.now, ramq, memory.level))
    ...
    while instructionsq > 0:  # Loop hasta que se acabe la cantidad de instrucciones pendientes.
        ...
        print('%s \nProceso en cola READY tiempo -> %d Cantidad instrucciones pendientes %d' % (name, env.now, instructionsq))
        ...
        with cpu.request() as req:  # Solicitud al procesador.
            ...
            print('%s \nProceso en estado RUNNING fue atendido en tiempo -> %d Cantidad ram %d, Instrucciones pendientes %d ram disponible %d' % (name, env.now, ramq, instructionsq, memory.level))

    # Retorna la memoria utilizada.
    yield memory.put(ramq)
    print('%s \nProceso TERMINATED salida -> %d cantidad ram devuelta %d, nueva cantidad de memoria disponible %d' % (name, env.now, ramq, memory.level))
    global totalTime
    totalTime += env.now - arrivalTime
    print('\nTiempo total %d' % (env.now - arrivalTime)) # Tiempo total en base al arrivalTime.
